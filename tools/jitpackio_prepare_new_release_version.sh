...
f1="jitpack.yml"
f2="pom.xml"
f3=$(ls -1tr local_maven_iocpiher_*.zip|tail -1|tr -d " ")
...
cur_m_version=$(echo "$f3" | sed -e 's#^.*local_maven_iocpiher_##'|sed -e 's#.zip$##')
...
sed -i -e 's#unzip local_maven_iocpiher_.*$#unzip local_maven_iocpiher_'"$next_m_version"'.zip#' "$f1"
